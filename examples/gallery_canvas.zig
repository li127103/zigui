//! zigui gallery - 自绘组件 (Canvas / DrawingArea)
//!
//! 演示:
//!   - Canvas 控件 + paint_fn 回调, 拿到 Renderer2D 直接绘制 (fillRect/fillRoundedRect/fillCircle/文本)
//!   - DrawingArea 控件 + draw_func 回调, 自定义绘制矩形画布
//!   - 用 g_frame 驱动简单动画 (无需外部计时器)
//!
//! 运行:  zig build run-gallery-canvas
...
const DrawingArea = zigui.drawing_area.DrawingArea;
...
pub fn main() !void {
    ...
    var app = try zigui.app.App.init(allocator, .{ .title = "zigui Gallery - Canvas", .width = 960, .height = 680 });
    defer app.deinit();
...
fn buildTree(alloc: std.mem.Allocator) !void {
    ...
    const title = try Label.create(alloc, "Canvas / DrawingArea", .{ .font_size = 22, .font_weight = 700, .color = math.Color.hex(0xF8FAFCFF) });
    ...
    try addCanvasCard(row, alloc, "Canvas: Progress", paintProgress);
    try addCanvasCard(row, alloc, "Canvas: Bar Chart", paintChart);
    try addCanvasCard(row, alloc, "Canvas: Shapes", paintShapes);

    // DrawingArea 卡片
    try addDrawingCard(row, alloc, "DrawingArea: Chart", drawChart);

    g_root = root;
    g_tree_alloc = alloc;
}

fn addCanvasCard(parent: *Container, alloc: std.mem.Allocator, label: []const u8, paint_fn: *const fn (*widget.Widget, *widget.PaintContext) void) !void {
...
fn addDrawingCard(parent: *Container, alloc: std.mem.Allocator, label: []const u8, draw_fn: *const fn (*widget.PaintContext, f32, f32, ?*anyopaque) void) !void {
    const card = try Container.create(alloc, .{
        .bg_color = math.Color.hex(0x1E293BFF), .corner_radius = 14, .direction = .column,
        .padding = .{ .left = 16, .top = 14, .right = 16, .bottom = 16 },
    });
    ...
    const da = try DrawingArea.create(alloc, .{ .content_width = 240, .content_height = 200 });
    da.setDrawFunc(draw_fn, null);
    da.base.layout_style.margin.top = 14;
    da.base.layout_style.flex_grow = 1;
    da.base.layout_style.width = .{ .auto = {} };
    da.base.layout_style.min_height = .{ .px = 200 };
    try card.base.addChild(alloc, &da.base);
}
...
// DrawingArea 绘制回调: 使用 cr.renderer 直接绘制
fn drawChart(cr: *widget.PaintContext, width: f32, height: f32, _: ?*anyopaque) void {
    const bw = width;
    const bh = height;
    cr.renderer.fillRoundedRect(.{ .x = 0, .y = 0, .width = bw, .height = bh }, 10, math.Color.hex(0x0F172AFF)) catch {};
    ...
    var idx: usize = 0;
    while (idx < bars.len) : (idx += 1) {
        const cx = slot * @as(f32, @floatFromInt(idx)) + (slot - bar_w) / 2;
        const bar_h = max_h * bars[idx];
        const cy = bh - bar_h - 10;
        cr.renderer.fillRoundedRect(.{ .x = cx, .y = cy, .width = bar_w, .height = bar_h }, 6, math.Color.hex(bar_colors[idx])) catch {};
    }
    styled_text.drawText(cr.renderer, cr.allocator, "drawing area", 12, 16, .{ .font_size = 14, .font_weight = 600, .color = math.Color.hex(0xF8FAFCFF) });
}
