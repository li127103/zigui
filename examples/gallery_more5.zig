//! zigui gallery - 窗口 / 菜单按钮 (More 5)
//!
//! 演示:
//!   - WindowHandle   窗口拖拽手柄
//!   - MenuButton     菜单按钮
//!
//! 运行:  zig build run-gallery-more5
...
const WindowHandle = zigui.window_handle.WindowHandle;
...
fn buildTree(alloc: std.mem.Allocator) !void {
    ...
    const title = try Label.create(alloc, "More (5)", .{ .font_size = 26, .font_weight = 700, .color = math.Color.hex(0xF8FAFCFF) });
    try root.base.addChild(alloc, &title.base);

    const sub = try Label.create(alloc, "WindowHandle / MenuButton", .{ .font_size = 13, .color = math.Color.hex(0x64748BFF) });
    try root.base.addChild(alloc, &sub.base);

    // WindowHandle: 拖拽手柄, 内含一个标签
    const handle = try WindowHandle.new(alloc);
    const handle_label = try Label.create(alloc, "⋮⋮ Drag window", .{ .font_size = 13, .color = math.Color.hex(0xF8FAFCFF) });
    handle.setChild(&handle_label.base);
    handle.base.layout_style.height = .{ .px = 36 };
    try addRow(alloc, root, "Handle", handle);

    // MenuButton
    const mb = try MenuButton.create(alloc, "Open Menu", .{ .on_click = onMenu });
    try addRow(alloc, root, "Menu", mb);

    g_root = root;
    g_tree_alloc = alloc;
}

fn onMenu(_: *MenuButton) void {}
