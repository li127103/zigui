...
pub const Table = struct {
    ...
    fn drawCell(self: *Table, ctx: *PaintContext, text: []const u8, x: f32, y: f32, color: math.Color, weight: u16) void {
        styled_text.drawText(ctx.renderer, ctx.allocator, text, x, y, .{ .font_size = self.font_size, .color = color, .font_weight = weight });
    }

    fn drawCellClipped(self: *Table, ctx: *PaintContext, text: []const u8, x: f32, y: f32, color: math.Color, clip: math.Rect(f32)) void {
        _ = styled_text.drawTextClipped(ctx.renderer, ctx.allocator, text, x, y, .{ .font_size = self.font_size, .color = color }, clip.width);
    }
